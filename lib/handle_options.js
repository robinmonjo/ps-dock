...
var Handler = function(args){
  ...
  this.settings = this.searchConfigFiles(this.parseConfigFile, function(){
    ...
    handler.options.command = parseRealCommand(handler.args);
    handler.options.logging = handler.options.stdout ? handler.parseStdout(handler.options.stdout) : undefined;
    return 1;
  });
}

var parseRealCommand = function(args) {
  var baseCommand = args[0];
  var commandArgs = [];
  for (i = 1; i < args.length; i++) {
    if(args[i].indexOf(" ") != -1) {
      //the is a space in the argument, surround it with quotes
      args[i] = "\"" + args[i] + "\"";
    }
    commandArgs.push(args[i]);
  }
  return baseCommand + " " + commandArgs.join(" ");
}
